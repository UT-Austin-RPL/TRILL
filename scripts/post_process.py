...
def segment_fcn(player, mode="step"):
    if mode == "reset":
        ...
        for label in OBS_EEF_KEYS:
            player._post_buffer[
                "observation/trajectory/{}".format(label)
            ] = collections.deque()
    ...
    if mode == "step":
        if player._demo_count < player._demo_length:
            ...
            for key in trajectory.keys():
                player._post_buffer["observation/trajectory/{}".format(key)].append(
                    np.copy(trajectory[key])
                )

    elif mode == "close":
        os.system(
            "cp {} {}".format(
                os.path.join(player._path, "demo.hdf5"),
                os.path.join(player._path, "demo_old.hdf5"),
            )
        )
        ...
        for label, buffer in player._post_buffer.items():
            ...
            if label in [
                "observation/trajectory/{}".format(key)
                for key in ["lf_foot_pos", "rf_foot_pos"]
            ]:
                del new_demo_file[label]
            new_demo_file.create_dataset(
...
def process_fcn(player, mode="step", dataname="data"):
    ...
    if mode == "step":
        ...
        new_demo_file = h5py.File(
            os.path.join(player._path, "demo_{}.hdf5".format(dataname)), "a"
        )

        new_demo_file.create_dataset(
            "subtask",
            data=np.round(original_demo_file["observation/subtask"]),
            compression="gzip",
            chunks=True,
            dtype="uint8",
        )
        ...
        for eef_key in OBS_EEF_KEYS:
            obs_group.create_dataset(
                eef_key,
                data=np.array(
                    original_demo_file["observation/trajectory/{}".format(eef_key)]
                ),
                compression="gzip",
                chunks=True,
                dtype="f",
        ...
        obs_joint_pos = np.column_stack(
            [
                original_demo_file["observation/joint_pos/{}".format(joint_key)]
                for joint_key in OBS_JOINT_KEYS
            ]
        )
        obs_joint_vel = np.column_stack(
            [
                original_demo_file["observation/joint_vel/{}".format(joint_key)]
                for joint_key in OBS_JOINT_KEYS
            ]
        )
        obs_group.create_dataset(
            "joint",
            data=np.concatenate(
                (np.cos(obs_joint_pos), np.sin(obs_joint_pos), obs_joint_vel), axis=1
            ),
            compression="gzip",
            chunks=True,
            dtype="f",
        ...
        obs_group.create_dataset(
            "state",
            data=np.array(original_demo_file["observation/state"]) / 15.0,
            compression="gzip",
            chunks=True,
            dtype="f",
        )
        ...
        act_discrete = np.column_stack(
            [
                locomtion_buffer,
                original_demo_file["action/gripper/right"],
                original_demo_file["action/gripper/left"],
            ]
        )
        ...
        obs_group.create_dataset(
            "action", data=obs_action, compression="gzip", chunks=True, dtype="f"
        )

        new_demo_file.create_dataset(
            "actions", data=act_concat, compression="gzip", chunks=True, dtype="f"
        )
...
def segmentation(path, env_type="door"):
    ...
    renderer = CV2Renderer(
        device_id=-1, sim=env.sim, width=400, height=300, cam_name=cam_name, gui=True
    )
    ...
    renderer_left = CV2Renderer(
        ...
        width=240,
        height=180,
        cam_name="robot0_robotleft",
        gui=False,
    )

    player.set_renderer(renderer)
    player.set_stereo_renderer(
        renderer_right=renderer_right, renderer_left=renderer_left
    )
...
def batch_subtask_data(path, env_type="door", subtask=0, dataname="data"):
    out_file = h5py.File(
        os.path.join(path, "{}_{}_{}.hdf5").format(dataname, env_type, subtask), "w"
    )
        ...
        for label, data in obs_data.items():
            if label not in ["right_rgb", "left_rgb"]:
                obs_grp.create_dataset(
                    label, data=data, compression="gzip", chunks=True, dtype="f"
                )
            else:
                obs_grp.create_dataset(
                    label, data=data, compression="gzip", chunks=True, dtype="uint8"
                )
...
def merge_data(path, env_type="door", subtask=0):
    out_file = h5py.File(
        os.path.join(path, "{}_{}_merged.hdf5").format(env_type, subtask), "w"
    )
    out_data = out_file.create_group("data")
    ...
    for dataset_path in dataset_paths:
        ...
        for demo in dataset["data"]:
            ...
            ep_grp.attrs["num_samples"] = dataset["data/{}".format(demo)].attrs[
                "num_samples"
            ]
...
if __name__ == "__main__":
    ...
    parser.add_argument("--path", type=str, default="default", help="")
    parser.add_argument("--mode", type=str, default="process", help="")
    parser.add_argument("--env", type=str, default="door", help="")
    parser.add_argument("--subtask", type=int, default=0, help="")
    parser.add_argument("--dataname", type=str, default="data", help="")
