...
from simulator.wrapper import EnvWrapper
...
def evaluate(
    path, gui, env_type, cam_name="upview", subtask=0, seed=0, save_path=None, **kwargs
):
    ...
    if env_type == "workbench2" and subtask == 1:
        ...
        target_pos = np.array(
            [0.22, -0.35, -0.0]
        )  # + np.random.normal(0, 0.02, size=3)
        ...
        while env.cur_time < 5.0:
            ...
            rh_input = geom.euler_to_rot(
                -0.6 * np.pi * np.sin(0.5 * np.pi * quat_phase) * np.array([1, 0, 0])
            )
            rh_target_pos = init_pos + (target_pos - init_pos) * np.sin(
                0.5 * np.pi * pos_phase
            )
    ...
    while not done:
        ...
        if env.cur_time > log_time:
            ...
            print(
                "Processing...\tSubtask: {},\tSeed: {},\tTime: {},\tSave to:{}".format(
                    subtask, seed, env.cur_time, save_path
                )
            )
...
if __name__ == "__main__":
    ...
    if args.save is not None:
        ...
        save_path = os.path.join(
            args.save, "{}_{}_{}_{}.mp4".format(seed, env_type, cam_name, subtask)
        )
    else:
        save_path = None
