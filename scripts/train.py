...
def train(config, device, info):
    ...
    if 'resume' in info.keys():
        load_path = os.path.join(config.train.output_dir, config.experiment.name, info['resume']['id'])
        if info['resume']['epoch'] is not None:
            resume_ckpt_path = os.path.join(load_path, 'models', 'epoch_{}.pth'.format(info['resume']['epoch']))
        else:
            resume_ckpt_path = os.path.join(load_path, 'models', 'model_best_training.pth')
...
def main():
    ...
    ext_cfg['train'].update({'data': os.path.join(cwd, 'datasets', '{}_{}_{}.hdf5'.format(args.data, args.env, args.subtask)),
                            'output_dir':os.path.join(cwd, 'save', args.exp, '{}_{}'.format(args.env, args.subtask))
                            })
    ...
    info = {}
    info['config'] = ext_cfg
    info['note'] = {'experiment': args.exp, 'env_name': args.env, 'subtask': args.subtask}
