...
class DoubleSupportStand(StateMachine):
    ...
    def first_visit(self):
        ...
        self._trajectory_managers[
            "floating_base"
        ].initialize_floating_base_interpolation_trajectory(
            self._sp.curr_time, self._end_time, com_pos_des, base_quat_des
        )

        # Update upper body task
        self._trajectory_managers["upper_body"].use_nominal_upper_body_joint_pos(
    ...
    def one_step(self):
        ...
        self._trajectory_managers["floating_base"].update_floating_base_desired(
            self._sp.curr_time
        )
        # Update Foot Task
