...
class DracoManipulationInterruptLogic(InterruptLogic):
    ...
    def b_walk_in_progress(self):
        if self._control_architecture.state_machine[
                        LocomanipulationState.BALANCE].walking_trigger:
    ...
    def process_interrupts(self):
        if self._standby:
            ...
            self._control_architecture._manipulation.rh_waypoint_pos = self._rh_waypoint_pos
            self._control_architecture._manipulation.rh_target_quat = self._rh_target_quat

            self._control_architecture._manipulation.lh_target_pos = self._lh_target_pos
            self._control_architecture._manipulation.lh_waypoint_pos = self._lh_waypoint_pos
            self._control_architecture._manipulation.lh_target_quat = self._lh_target_quat
