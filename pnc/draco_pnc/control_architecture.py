...
class DracoManipulationControlArchitecture(ControlArchitecture):
    def __init__(self, robot, config):
        ...
        self._dcm_tm.nominal_forward_step = walking_config["Initial Motion"][
            "Forward Step"
        ]
        self._dcm_tm.nominal_backward_step = walking_config["Initial Motion"][
            "Backward Step"
        ]
        self._dcm_tm.nominal_turn_radians = walking_config["Initial Motion"][
            "Trun Angle"
        ]
        ...
        self._state_machine[LocomanipulationState.STAND] = DoubleSupportStand(
            LocomanipulationState.STAND,
            self._trajectory_managers,
            self._hierarchy_managers,
            self._reaction_force_managers,
            self._robot,
        )
        self._state_machine[LocomanipulationState.STAND].end_time = walking_config[
            "Duration"
        ]["Initial Stand"]
        self._state_machine[LocomanipulationState.STAND].rf_z_max_time = walking_config[
            "Duration"
        ]["RF Z Max"]
        self._state_machine[
            LocomanipulationState.STAND
        ].com_height_des = walking_config["Initial Motion"]["COM Height"]

        self._state_machine[LocomanipulationState.BALANCE] = DoubleSupportBalance(
        ...
        self._state_machine[LocomanipulationState.LF_SWING] = SingleSupportSwing(
            LocomanipulationState.LF_SWING,
            self._trajectory_managers,
            Footstep.LEFT_SIDE,
            self._robot,
        )
        ...
        self._state_machine[LocomanipulationState.RF_SWING] = SingleSupportSwing(
            LocomanipulationState.RF_SWING,
            self._trajectory_managers,
            Footstep.RIGHT_SIDE,
            self._robot,
        )
        ...
        self._manipulation = Manipulation(
            LocomanipulationState.DH_MANIPULATION,
            self._trajectory_managers,
            self._hierarchy_managers,
            self._reaction_force_managers,
            self._robot,
        ...
        self._manipulation.rh_target_pos = np.array(
            manipulation_config["Initial Target"]["RH Pos"]
        )
        self._manipulation.rh_target_quat = np.array(
            manipulation_config["Initial Target"]["RH Quat"]
        )
        self._manipulation.lh_target_pos = np.array(
            manipulation_config["Initial Target"]["LH Pos"]
        )
        self._manipulation.lh_target_quat = np.array(
            manipulation_config["Initial Target"]["LH Quat"]
        )
        self._manipulation.trans_duration = manipulation_config["Duration"][
            "Reaching Trans"
        ]
