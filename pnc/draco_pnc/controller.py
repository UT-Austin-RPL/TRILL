...
class DracoManipulationController(object):
    def __init__(self, tci_container, robot, config):
        ...
        self._ihwbc = IHWBC(
            self._sf, self._sa, self._sv, self._config["Simulation"]["Save Data"]
        )
        ...
        if self._config["Whole-Body Contol"]["Use Torque Limit"]:
            self._ihwbc.trq_limit = np.dot(self._sa[:, 6:], self._robot.joint_trq_limit)
        self._ihwbc.lambda_q_ddot = self._config["Whole-Body Contol"]["Reguralization"][
            "Q ddot"
        ]
        self._ihwbc.lambda_rf = self._config["Whole-Body Contol"]["Reguralization"][
            "RF"
        ]

        # Initialize Joint Integrator
        self._joint_integrator = JointIntegrator(
            robot.n_a, self._config["Simulation"]["Control Period"]
        )
        self._joint_integrator.pos_cutoff_freq = self._config["Whole-Body Contol"][
            "Integration"
        ]["Cutoff Frequency"]["Joint Pos"]
        self._joint_integrator.vel_cutoff_freq = self._config["Whole-Body Contol"][
            "Integration"
        ]["Cutoff Frequency"]["Joint Vel"]
        self._joint_integrator.max_pos_err = self._config["Whole-Body Contol"][
            "Integration"
        ]["Joint Pos Error Max"]
    ...
    def get_command(self):
        ...
        joint_trq_cmd, joint_acc_cmd, rf_cmd = self._ihwbc.solve(
            self._tci_container.task_list,
            self._tci_container.contact_list,
        ...
        joint_vel_cmd, joint_pos_cmd = self._joint_integrator.integrate(
            joint_acc_cmd, self._robot.joint_velocities, self._robot.joint_positions
        )
