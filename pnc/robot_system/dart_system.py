...
class DartRobotSystem(RobotSystem):
    ...
    def _config_robot(self, urdf_file, package_dir):
        ...
        self._joint_pos_limit = np.stack(
            [self._skel.getPositionLowerLimits(), self._skel.getPositionUpperLimits()],
            axis=1,
        )[self._n_floating : self._n_floating + self._n_a, :]
        self._joint_vel_limit = np.stack(
            [self._skel.getVelocityLowerLimits(), self._skel.getVelocityUpperLimits()],
            axis=1,
        )[self._n_floating : self._n_floating + self._n_a, :]
        self._joint_trq_limit = np.stack(
            [self._skel.getForceLowerLimits(), self._skel.getForceUpperLimits()], axis=1
        )[self._n_floating : self._n_floating + self._n_a, :]

    def get_q_idx(self, joint_id):
        if type(joint_id) is list:
            return [self._joint_id[id].getIndexInSkeleton(0) for id in joint_id]
        else:
    ...
    def get_joint_idx(self, joint_id):
        if type(joint_id) is list:
            ...
            return self._joint_id[joint_id].getIndexInSkeleton(0) - self._n_floating
    ...
    def create_cmd_ordered_dict(self, joint_pos_cmd, joint_vel_cmd, joint_trq_cmd):
        ...
        for k, v in self._joint_id.items():
            joint_idx = self._joint_id[k].getIndexInSkeleton(0) - self._n_floating
    ...
    def update_system(
        ...
        for (p_k, p_v), (v_k, v_v) in zip(joint_pos.items(), joint_vel.items()):
    ...
    def get_com_lin_vel(self):
        return np.copy(
            self._skel.getCOMLinearVelocity(
                dart.dynamics.Frame.World(), dart.dynamics.Frame.World()
            )
        )

    def get_com_lin_jacobian(self):
        return np.copy(self._skel.getCOMLinearJacobian(dart.dynamics.Frame.World()))
    ...
    def get_link_iso(self, link_id):
        ...
        ret[0:3, 3] = self._link_id[link_id].getCOM(dart.dynamics.Frame.World())
        return np.copy(ret)

    def get_link_vel(self, link_id):
        return np.copy(
            self._link_id[link_id].getCOMSpatialVelocity(
                dart.dynamics.Frame.World(), dart.dynamics.Frame.World()
            )
        )
