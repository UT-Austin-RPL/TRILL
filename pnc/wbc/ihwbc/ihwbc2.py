    ...
    def solve(
        ...
        if len(internal_constraint_list) > 0:
            if b_transmission_constraint:
                ...
                sa_trc = (self._sa)[:, 6:]
                sa_trc_bar = weighted_pinv(sa_trc, self._mass_matrix_inv[6:, 6:])
                ...
                if b_transmission_constraint:
                    eq_trans_mat = np.concatenate(
                        (
                            ...
                            np.dot(
                                (-self._sd + self._sv), contact_jacobian.transpose()
                            ),
                        ),
                        axis=1,
                    )
                    eq_trans_vec = np.dot(
                        (-self._sd + self._sv), (self._coriolis + self._gravity)
                    )
                else:
        ...
        if b_internal_constraint:
            if b_transmission_constraint:
                eq_mat = np.concatenate((eq_floating_mat, eq_trans_mat), axis=0)
                eq_vec = np.concatenate((eq_floating_vec, eq_trans_vec), axis=0)
            else:
