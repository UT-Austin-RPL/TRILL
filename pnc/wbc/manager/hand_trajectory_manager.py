...
class HandTrajectoryManager(object):
    def __init__(
        self, pos_task, ori_task, robot, target_vel_max=None, trajectory_mode=None
    ):
        ...
        if self._ori_task is not None:
            self._ori_task.pos_des = np.array(
                [0, 0, 0, 1]
            )  # prevent error in quat basic task
        self._robot = robot
    ...
    def update_desired(self, target_hand_iso):
        ...
        if self._ori_task is not None:
            self._ori_task.update_desired(
                hand_ori_des, hand_ang_vel_des, hand_ang_acc_des
            )
    ...
    def initialize_hand_pose(self, start_time, duration, target_hand_iso):
    ...
    def initialize_keypoint_hand_trajectory(
        self, start_time, duration, keypoint_pos, target_hand_iso
    ):
        self._start_moving_time = start_time
        self._moving_duration = duration
    ...
    def update_hand_pose(self, current_time):
        ...
        hand_pos_des, hand_vel_des, hand_acc_des = np.zeros(3), np.zeros(3), np.zeros(3)
        hand_quat_des, hand_ang_vel_des, hand_ang_acc_des = (
            np.zeros(4),
            np.zeros(3),
            np.zeros(3),
        )
        ...
        if self._trajectory_mode == "interpolation":
            for i in range(3):
                hand_pos_des[i] = interpolation.smooth_changing(
                    self._init_hand_pos[i],
                    self._target_hand_pos[i],
            ...
            LPF_CUTOFF = 25
            WBC_PERIOD = 0.002
            alpha = (2 * np.pi * LPF_CUTOFF * WBC_PERIOD) / (
                1 + 2 * np.pi * LPF_CUTOFF * WBC_PERIOD
            )
    ...
    def update_keypoint_hand_trajectory(self, current_time):
        ...
        s = (current_time - self._start_moving_time) / (0.5 * self._moving_duration)
