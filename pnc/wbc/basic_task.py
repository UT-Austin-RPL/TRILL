    ...
    def update_cmd(self):
        if self._task_type == "CAM":
            ...
            if self._b_data_save:
                ...
                self._data_saver.add(self._target_id + '_local_vel_des', local_vel_des.copy())
                self._data_saver.add(self._target_id + '_local_vel', local_vel_act.copy())
                self._data_saver.add('w_' + self._target_id, self._w_hierarchy)
            self._op_cmd = self._acc_des + rot_w_link @ (np.array(self._kd) * local_vel_err)
            return
            ...
            quat_act_temp = geom.prevent_quat_jump(quat_des_temp,
                                                   quat_act_temp)
    ...
    def update_jacobian(self):
        if self._task_type == "CAM":
            self._jacobian = np.dot(
                np.linalg.inv(self._robot.get_Ig()), 
                self._robot.get_Ag())\
                [0:3, :]
