...
def get_body_pos_vel(sim, robot):
    ...
    state = {
        "body_pos": {
            ...
            "quat": np.copy(data.qpos[joint_qposadr + 3 : joint_qposadr + 7])[
                [1, 2, 3, 0]
            ],
        },
        "body_vel": {
...
def set_motor_impedance(sim, robot, command, kp, kd):
    ...
    for (pnc_key, pos_des), (_, vel_des), (_, trq_des) in zip(
        ...
        if type(kd) == dict:
            ...
            kd_val = kd
        trq_applied[actuator_id] = (
            trq_des + kp_val * (pos_des - joint_pos) + kd_val * (vel_des - joint_vel)
        )
...
def set_ball_pos(sim, robot, state):
    ...
    for pnc_key, pos_des in state["joint_pos"].items():
        ...
        joint_id = mujoco.mj_name2id(model, mujoco.mjtObj.mjOBJ_JOINT, mujoco_joint_key)
        joint_qposadr = model.jnt_qposadr[joint_id]
        for qposadr_idx, val_idx in enumerate([3, 0, 1, 2]):
            pos_applied[joint_qposadr + qposadr_idx] = pos_des[val_idx]
    data.qpos[list(pos_applied.keys())] = list(pos_applied.values())
...
def set_motor_pos_vel(sim, robot, state):
    ...
    pos_applied = OrderedDict()
    vel_applied = OrderedDict()
    for (pnc_key, pos_des), (_, vel_des) in zip(
...
def set_body_pos_vel(sim, robot, state):
    ...
    data.qvel[joint_qveladr : joint_qveladr + 6] = np.concatenate(
        (state["body_vel"]["pos"], state["body_vel"]["rpy"])
    )
...
def set_ball_pos_vel(sim, robot, state):
    ...
    for (pnc_key, pos_des), (_, vel_des) in zip(
        ...
        for qposadr_idx, val_idx in enumerate([3, 0, 1, 2]):
            pos_applied[joint_qposadr + qposadr_idx] = pos_des[val_idx]
        for idx in range(3):
            vel_applied[joint_qveladr + idx] = vel_des[idx]
    data.qpos[list(pos_applied.keys())] = list(pos_applied.values())
    data.qvel[list(vel_applied.keys())] = list(vel_applied.values())
...
def get_sensor_data(sim, robot):
    ...
    base_id = mujoco.mj_name2id(
        model, mujoco.mjtObj.mjOBJ_XBODY, robot.naming_prefix + "base"
    )
    lh_eef_id = mujoco.mj_name2id(
        model, mujoco.mjtObj.mjOBJ_XBODY, robot.naming_prefix + "left_hand"
    )
    rh_eef_id = mujoco.mj_name2id(
        model, mujoco.mjtObj.mjOBJ_XBODY, robot.naming_prefix + "right_hand"
    )
    ...
    base_com_lin_vel = np.dot(
        base_com_jacp, data.qvel
    )  # np.copy(data.cvel[base_com_id])[:3]
    base_com_ang_vel = np.dot(
        base_com_jacr, data.qvel
    )  # np.copy(data.cvel[base_com_id])[3:]
    ...
    lh_eef_pos = np.dot(
        rot_world_basejoint.transpose(), data.xpos[lh_eef_id] - base_com_pos
    )
    rh_eef_pos = np.dot(
        rot_world_basejoint.transpose(), data.xpos[rh_eef_id] - base_com_pos
    )
...
def get_trajectory(sim, robot):
    ...
    lf_foot_id = mujoco.mj_name2id(
        model, mujoco.mjtObj.mjOBJ_XBODY, robot.naming_prefix + "left_foot"
    )
    rf_foot_id = mujoco.mj_name2id(
        model, mujoco.mjtObj.mjOBJ_XBODY, robot.naming_prefix + "right_foot"
    )
    ...
    lf_foot_pos = np.dot(
        rot_world_basejoint.transpose(), data.xpos[lf_foot_id] - base_com_pos
    )
    rf_foot_pos = np.dot(
        rot_world_basejoint.transpose(), data.xpos[rf_foot_id] - base_com_pos
    )

    lh_eef_rot = np.dot(
        rot_world_basejoint.transpose(),
        geom.quat_to_rot(data.xquat[lh_eef_id][[1, 2, 3, 0]]),
    )
    rh_eef_rot = np.dot(
        rot_world_basejoint.transpose(),
        geom.quat_to_rot(data.xquat[rh_eef_id][[1, 2, 3, 0]]),
    )
    lf_foot_rot = np.dot(
        rot_world_basejoint.transpose(),
        geom.quat_to_rot(data.xquat[lf_foot_id][[1, 2, 3, 0]]),
    )
    rf_foot_rot = np.dot(
        rot_world_basejoint.transpose(),
        geom.quat_to_rot(data.xquat[rf_foot_id][[1, 2, 3, 0]]),
    )
...
def transform_local_trajectory(sim, robot, global_trajectory):
    ...
    rot_world_basejoint = geom.quat_to_rot(base_com_quat)
    lh_eef_pos = np.dot(
        rot_world_basejoint.transpose(), global_trajectory["lh_eef_pos"] - base_com_pos
    )
    rh_eef_pos = np.dot(
        rot_world_basejoint.transpose(), global_trajectory["rh_eef_pos"] - base_com_pos
    )
    lf_foot_pos = np.dot(
        rot_world_basejoint.transpose(), global_trajectory["lf_foot_pos"] - base_com_pos
    )
    rf_foot_pos = np.dot(
        rot_world_basejoint.transpose(), global_trajectory["rf_foot_pos"] - base_com_pos
    )

    lh_eef_rot = np.dot(
        rot_world_basejoint.transpose(),
        geom.quat_to_rot(global_trajectory["lh_eef_quat"]),
    )
    rh_eef_rot = np.dot(
        rot_world_basejoint.transpose(),
        geom.quat_to_rot(global_trajectory["rh_eef_quat"]),
    )
    lf_foot_rot = np.dot(
        rot_world_basejoint.transpose(),
        geom.quat_to_rot(global_trajectory["lf_foot_quat"]),
    )
    rf_foot_rot = np.dot(
        rot_world_basejoint.transpose(),
        geom.quat_to_rot(global_trajectory["rf_foot_quat"]),
    )
