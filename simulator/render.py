...
def getVRPose(socket):
    ...
    hmd_mat = np.frombuffer(
        message, dtype=np.float32, count=9, offset=3 * FLOAT_SIZE
    ).reshape((3, 3))
    left_pos = np.frombuffer(message, dtype=np.float32, count=3, offset=12 * FLOAT_SIZE)
    left_mat = np.frombuffer(
        message, dtype=np.float32, count=9, offset=15 * FLOAT_SIZE
    ).reshape((3, 3))
    right_pos = np.frombuffer(
        message, dtype=np.float32, count=3, offset=24 * FLOAT_SIZE
    )
    right_mat = np.frombuffer(
        message, dtype=np.float32, count=9, offset=27 * FLOAT_SIZE
    ).reshape((3, 3))
    ...
    left_trigger = np.frombuffer(
        message, dtype=np.float32, count=1, offset=36 * FLOAT_SIZE
    )
    left_bump = np.frombuffer(
        message, dtype=np.float32, count=1, offset=37 * FLOAT_SIZE
    )
    left_button = np.frombuffer(
        message, dtype=np.float32, count=1, offset=38 * FLOAT_SIZE
    )
    left_pad = np.frombuffer(message, dtype=np.float32, count=1, offset=39 * FLOAT_SIZE)

    right_trigger = np.frombuffer(
        message, dtype=np.float32, count=1, offset=40 * FLOAT_SIZE
    )
    right_bump = np.frombuffer(
        message, dtype=np.float32, count=1, offset=41 * FLOAT_SIZE
    )
    right_button = np.frombuffer(
        message, dtype=np.float32, count=1, offset=42 * FLOAT_SIZE
    )
    right_pad = np.frombuffer(
        message, dtype=np.float32, count=1, offset=43 * FLOAT_SIZE
    )
    ...
    transformed_left[2] = -transformed_left[2] + 0.2
    transformed_left[0] += 0.15
    transformed_right = -local_right_pos[[2, 0, 1]]
    transformed_right[0] += 0.15
    transformed_right[2] = -transformed_right[2] + 0.2
    ...
    return (
        transformed_left,
        transformed_right,
        convert_orientation(left_orientation),
        convert_orientation(right_orientation),
        left_trigger,
        left_bump,
        left_button,
        left_pad,
