...
class WheeledArmController(object):
    def __init__(self) -> None:
        ...
        self._left_gripper_target = {
            "joint_pos": OrderedDict(),
            "joint_vel": OrderedDict(),
            "joint_trq": OrderedDict(),
        }
        self._right_gripper_target = {
            "joint_pos": OrderedDict(),
            "joint_vel": OrderedDict(),
            "joint_trq": OrderedDict(),
        }
        self._hand_target = {"left": OrderedDict(), "right": OrderedDict()}
        self._sensor_data = OrderedDict()
        self._init_state = OrderedDict()
        # self._init_state.update(config['Simulation']['Initial State'])
        self._init_state = {
            "Joint Pos": {
                "base_wheel_{}".format(key): 0.0 for key in ["fl", "fr", "bl", "br"]
            },
            "Body Pos": [0.0, 0.0, 0.0],
            "Body Quat": [0.0, 0.0, 0.0, 1.0],
        }
        self._init_state["Joint Pos"].update(
            {
                "left_joint_{}".format(idx): value
    ...
    def reset(self):
        ...
        self._robot_target["joint_trq"].update(
            {key: 0.0 for key in self._robot_target["joint_pos"].keys()}
        )
        self._robot_target["body_pos"].update(
            {
                "pos": np.array(self._init_state["Body Pos"]),
        ...
        self._right_gripper_target["joint_vel"].update(
            {key: 0.0 for key in self._right_gripper_target["joint_pos"].keys()}
        )
        self._right_gripper_target["joint_trq"].update(
            {key: 0.0 for key in self._right_gripper_target["joint_pos"].keys()}
        )

        self._left_gripper_target["joint_pos"].update({"gripper": 0.0})
        self._left_gripper_target["joint_vel"].update(
            {key: 0.0 for key in self._left_gripper_target["joint_pos"].keys()}
        )
        self._left_gripper_target["joint_trq"].update(
            {key: 0.0 for key in self._left_gripper_target["joint_pos"].keys()}
        )

        self._hand_target["right"].update(
            {
                "pos": np.array([0.3, -0.3, 0.3]),
                "quat": np.array([0.0, 0.70710678, 0.0, -0.70710678]),
            }
        )
        self._hand_target["left"].update(
            {
                "pos": np.array([0.3, 0.3, 0.3]),
                "quat": np.array([0.0, 0.70710678, 0.0, -0.70710678]),
            }
...
class DracoController(object):
    ...
    def reset(self, initial_pos=None):
        ...
        self._hand_target.update(
            {
                "right_pos": np.copy(self._init_action["trajectory"]["right_pos"]),
    ...
    def update_trajectory(self, tajectory, locomotion=None):
        ...
        self._hand_target.update(
            {
                key: np.copy(value)
                for key, value in tajectory.items()
                if key in self._hand_target.keys()
            }
        )
        ...
        calmped_local_rh_target_pos = np.clip(
            self._hand_target["right_pos"],
            self._interface.config["Manipulation"]["Workspace"]["RH Min"],
            self._interface.config["Manipulation"]["Workspace"]["RH Max"],
        )
        calmped_local_lh_target_pos = np.clip(
            self._hand_target["left_pos"],
            self._interface.config["Manipulation"]["Workspace"]["LH Min"],
            self._interface.config["Manipulation"]["Workspace"]["LH Max"],
        )
        rh_target_pos = (
            np.dot(rot_world_basejoint, calmped_local_rh_target_pos) + base_com_pos
        )
        lh_target_pos = (
            np.dot(rot_world_basejoint, calmped_local_lh_target_pos) + base_com_pos
        )

        rh_target_rot = rot_world_basejoint @ geom.quat_to_rot(
            self._hand_target["right_quat"]
        )
        lh_target_rot = rot_world_basejoint @ geom.quat_to_rot(
            self._hand_target["left_quat"]
        )
