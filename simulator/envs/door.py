...
STD_INIT_YAW = {0: 0.2,
                1: 0.07,
                2: 0.07}
MEAN_INIT_HINGE = {0: 0.0,
                1: 0.00,
                2: 0.1}
STD_INIT_HINGE = {0: 0.0,
                1: 0.00,
                2: 0.025}
...
class DoorEnv(BaseEnv):
    ...
    def reset(self, initial_pos=None, subtask=0, **kwargs):
        if initial_pos is None:
            self._init_robot_states = {'pos': np.random.normal(MEAN_INIT_POS[subtask], STD_INIT_POS[subtask], size=3),
                                       'yaw': np.random.normal(MEAN_INIT_YAW[subtask], STD_INIT_YAW[subtask])}
        else:
            self._init_robot_states = initial_pos
        if subtask == 2:
            self._hinge_joint_random = np.max((np.random.normal(MEAN_INIT_HINGE[subtask], STD_INIT_HINGE[subtask]), 0.08))
        else:
    ...
    def _load_model(self):
        ...
        self.door = DoorObject(
                        ...
                        friction=0.0,
                        damping=0.1)
        self.walls = {'wall{}'.format(idx): WallObject(name='wall{}'.format(idx)) for idx in range(2)}
